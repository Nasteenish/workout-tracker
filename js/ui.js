...
export const UI = {
    ...
    _renderExercise(exVMorEx, weekNumOrUndef, dayNumOrUndef, choiceKeyOrUndef, groupIdx) {
        ...
        if (!_uniCat) {
            for (let _ci = 0; _ci < EXERCISE_DB.length; _ci++) {
                if (EXERCISE_DB[_ci].name === _attrName || EXERCISE_DB[_ci].nameRu === _attrNameRu) {
                    _uniCat = EXERCISE_DB[_ci].category; break;
                }
            }
        }
    ...
    _buildSetRowVM(ex, setIdx, weekNum, dayNum) {
        ...
        if (isCompleted && log.weight > 0 && log.reps > 0) {
            isPR = Analytics.isAllTimeBest(Storage.getLogExerciseId(ex.id), log.weight, log.reps, log.equipmentId || eqId || null, weekNum, dayNum);
        }

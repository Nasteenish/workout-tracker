...
export const EquipmentManager = {
    ...
    loadBrandEquipment(brand, exerciseType) {
        ...
        itemsPromise.then(function(items) {
            ...
            for (var j = 0; j < items.length; j++) {
                var key = items[j].id;
                if (!seenIds[key]) { seenIds[key] = true; unique.push(items[j]); }
            }
            items = unique;
            var html = '';
    ...
    _exerciseTypeMap: {
        ...
        'iso-lateral row': 'seated_row', 'iso-lateral high row': 'high_row',
        'iso-lateral low row': 'low_row', 'low row': 'low_row',

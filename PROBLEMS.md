...
### 9. Дублирование навигации «назад» (8+ копий)

**Где:** `js/app.js`, ~строки 2414–2530

**Проблема:** Один и тот же паттерн back-анимации скопирован 8+ раз:
```javascript
if (target.id === 'btn-back-XXX' || target.closest('#btn-back-XXX')) {
    app.style.transition = 'transform 0.18s ease-in, opacity 0.18s ease-in';
    app.style.transform = 'translateX(40px)';
    app.style.opacity = '0';
    setTimeout(() => { ...; location.hash = TARGET; }, 190);
}
```

**Решение:** Один метод `_navigateBack(targetHash)` + `data-back-target` атрибут. Убирает ~150 строк из app.js.
...
| 9 | Дублирование навигации назад (8 копий) | **Низкая** | Среднее | 🟡 P1 |
...
- Вынести навигацию назад в `_navigateBack()` (#9) — **20 минут**, убирает ~150 строк из app.js

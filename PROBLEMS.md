...
### ~~19. Нет валидации пользовательского ввода~~ ✅ РЕШЕНО

**Что сделано:** Добавлены `parseWeight(value)` и `parseReps(value)` в `utils.js` — парсинг с clamp `[0, 999]`. `parseWeight` поддерживает запятую как десятичный разделитель, округляет до 2 знаков. `parseReps` возвращает целое число. Заменены все 6 точек парсинга пользовательского ввода: `app.js` (_saveDebounced, complete-btn), `storage.js` (saveSegReps, saveSegWeight), `profile-manager.js` (checkin weight).

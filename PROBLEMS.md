...
### 4. Одноразовые миграции в `App.init()` — 110 строк хардкода

**Где:** `js/app.js`, строки 15–109

**Проблема:** 4 блока одноразовых data-fix с вложенными циклами по localStorage. Хардкодят конкретные ID (`eq_1773590540310`, `D2E1`), имена брендов (`precor`). Работают с localStorage напрямую. Дублирование: `_cleanOrphanedLogEntries()` (строки 276-308) делает то же что миграция в init.

**Решение:** Вынести в `migrations.js` с версионированием. Старые (уже выполненные у всех) — удалить.
